...
pig1 = trimdat[pigID == "tejonM302"]
pig1$datetime
cras = crop(ras, extent(c(xmin=min(pig1$longitude), xmax=max(pig1$longitude),
													ymin=min(pig1$latitude), ymax=max(pig1$latitude))))
ll = rasterToPoints(cras, function(x) x == 1)[, c("x", "y"), drop=FALSE]
...
parallel_pigctmc = function(pignm, anal_params, allpaths, studynm){
  ...
  loc_stack = process_covariates(locvars, studynm, extobj, 
                                 min(tdat$datetime), max(tdat$datetime), 
                                 ext="loc", landcovertypes = landcovertypes,
                                 croptypes=croptypes)
  
  grad_stack = process_covariates(gradvars, studynm, extobj,
                                 min(tdat$datetime), max(tdat$datetime), 
                                 ext="grad", landcovertypes = landcovertypes[landcovertypes != "wetland"])
  
  gradxy_stack = process_covariates(gradxyvar, studynm, extobj,
                                 min(tdat$datetime), max(tdat$datetime),
  ...
  #glmlist[[pignm]] = as.data.table(pigglmdat)

}

# TODO: FIX BUFFER
glmlist = mclapply(unqpigs[1], parallel_pigctmc, anal_params, allpaths, studynm, 
											mc.cores=anal_params$cores)
#names(glmlist) = unqpigs
...
# A bit ugly, but dropping wetland and canopycover tso we don't get duplicates. Will probably want to make this cleaner
