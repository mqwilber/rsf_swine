...
pig1 = dat[pigID == "20125txcamp"][1:100]
...
```{r}
set.seed(123)
constr = list(lower=c(-4, -4), upper=c(4, 4))
...
```{r,message=FALSE}
predTime = seq(min(pig1$datetime), max(pig1$datetime), 200)
...
Hmmmm...this is going to take some tuning.  Another option would be to focus on the discrete time approach to easily account for state switching and the influence of habitat covariates on behavior. In this case, we need even intervals, which is inherently accounted for in the continuous time setting.  However, this might not be that limiting.  