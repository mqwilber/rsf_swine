...
The study IDs are unique identifiers for different studies which are describes in the file `data_descriptions.docx`.


## Summary statistics of data

Some basic summary statistics for each study.
...
Generally, we currently don't have weight data for the studies from the movement study.  

```{r}
sum(wdat$number_with_weight)
```

We have weight data for 220 pigs.
...
Look that the distribution of collaring times for the different studies.  Note that `study == "canada"` has not been cleaned.
...
Which pigs have runs of at least 500 consecutive fixes than are less or equal to 120 minutes apart?
