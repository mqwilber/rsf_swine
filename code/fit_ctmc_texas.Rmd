...
dat  = pig1[1:100, list(x=longitude, y=latitude, datetime=datetime)]
predinterp = continuous_path(dat, "15 mins", "interp")

plot(dat[, list(x, y)], type="b")
points(predinterp$paths[[1]], cex=0.1, col="red")

predbfreq = continuous_path(dat, "15 mins", "bspline_freq", impute=3, df=100, sigma.fixed = 0.001)
...
predbbayes = continuous_path(dat, "15 mins", "bspline_bayes", impute=3, sigma.fixed = 0.001, df=190)
...
source("pigfxns.R")
dat = pig1[1:100, list(x=longitude, y=latitude, datetime=datetime)]
glm_data = fit_ctmcmodel(dat, loc_stackproj, grad_stackproj, method="interp", impute=4,
...
fits = list()
for(i in 1:4){
  fits[[i]] = glm(z ~ croplayer_loc + crw + elevation_grad,
...
Depending on the scale of the grid cells and the uncertainty in the basis function estimate of movement, we can really change our results regarding the significance of location predictors.  This makes a lot of sense if you think about
