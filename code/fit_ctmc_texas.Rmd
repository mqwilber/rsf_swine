...
ppigs = function(i, allpigs, numpoints, buffer, timestep){
  ...
  gradx_stack = stack(lapply(gradxy_stack, function(m) m$xgrad))
  grady_stack = stack(lapply(gradxy_stack, function(m) m$ygrad))
...
ppigs(unqpig[1], longdat2, 500, 0.002, "15 mins")

#unqpig = unique(longdat2$pigID)
#allglmdat = mclapply(1:length(unqpig), ppigs, longdat2, 500, 0.002, "15 mins", mc.cores=4)
...
tg = process_glmdata(allglmdat,
                            c("temperature", "ndvi.*loc", "ndvi.*grad", "forest.*loc", "crop.*loc", "openwater.*loc", "grain.*loc"),
                            c("temperature_loc", "ndvi_loc", "ndvi_grad", "forest_loc", "crop_loc", "water_loc", "grain_loc"),
                            c("my", "my", "my", "y", "y", "y", "y"))

tgform = lapply(tg, function(x) x[, c('z', 'pigID', 'temperature_loc', 'ndvi_loc', 'ndvi_grad', 
                                      'forest_loc', 'crop_loc', 'water_loc', 'grain_loc', 'imputeID', 
...
```{r}
library(lme4)
fit = glmer(z ~ forest_loc + scale(ndvi_loc) + scale(ndvi_grad) + scale(crw) + (forest_loc | pigID),
...
In general, pigs tend to move slower in forest