data{
    ...
    for(i in 2:N){
        ...
        prev_vectx[1] = obs_vals[i - 1, 1]; // x location
        prev_vecty[1] = obs_vals[i - 1, 2]; // y location
        ...
        full_trajx[1] = obs_vals[i, 1]; // x location
        full_trajx[2] = true_velx[i]; // Unobserved x vel

        full_trajy[1] = obs_vals[i, 2]; // observed y location
        full_trajy[2] = true_vely[i]; // unobserved y vel

        // Get the likelihood. Independent for each direction
